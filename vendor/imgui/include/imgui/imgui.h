...
namespace ImGui
{
    ...
    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your gui, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!
...
// Note: if you are trying to check whether your mouse should be dispatched to Dear ImGui or to your gui, you should use 'io.WantCaptureMouse' instead! Please read the FAQ!
...
struct ImGuiIO
{
    ...
    IMGUI_API void  AddMousePosEvent(float x, float y);                     // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. gui not focused and not hovered)
    ...
    ImU16       MouseClickedLastCount[5];           // Count successive number of clicks. Stays valid after mouse release. SetToBuffer after another click is done.
...
struct ImGuiPlatformIO
{
    ...
    // - Updated by: gui/backend. Update every frame to dynamically support changing monitor or DPI configuration.
